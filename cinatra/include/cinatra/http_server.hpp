...
namespace cinatra {
    ...
    public:
        ...
        {
            http_cache::get().set_cache_max_age(86400);    //24小时
            init_conn_callback();
        }
        ...
        bool port_in_use(unsigned short port) {
            ...
            io_service svc;
            tcp::acceptor acept(svc);    //used for accepting new socket connections
        ...
        //监听（路程：打开连接>设置选项>绑定端口>开始监听>异步接收请求消息>设置接收选项>解析请求头>根据消息类型解析消息体）
        std::pair<bool, std::string> listen(const boost::asio::ip::tcp::resolver::query & query) {
            ...
            for (; endpoints != boost::asio::ip::tcp::resolver::iterator(); ++endpoints) {
                ...
                try {
                    ...
                    acceptor->listen();    //Place the socket into the state where it will listen for new connections
        ...
        //这三个函数应该是无用的，都返回 -1
        intptr_t run_one() {    //intptr_t 就是 size_t 类型
        ...
        //服务器绑定client可以请求的url，以及url对应的处理函数
    ...
    private:
        ...
        void set_static_res_handler()
        {
            set_http_handler<POST,GET>(STATIC_RESOURCE, [this](request& req, response& res){
                if (download_check_) {    //download 是对 client 来说的
                ...
                switch (state) {
                    case cinatra::data_proc_state::data_begin:    //如果是刚开始就写 header
                    {
                        ...
                        if(is_small_file(in.get(),req)){    //如果服务器文件小于 5M 直接发送给 client
                        ...
                        break;
                    case cinatra::data_proc_state::data_continue:    //如果继续就写内容
                    {
                        if (transfer_type_ == transfer_type::CHUNKED)
                        ...
                        break;
                    case cinatra::data_proc_state::data_end:    //如果接收完了就写完成
                    {
        ...
        void write_chunked_header(request& req, std::shared_ptr<std::ifstream> in, std::string_view mime) {
            ...
            res_content_header += std::string("\r\n") + std::string("Accept-Ranges: bytes");    //表示服务器支持范围请求
            if (static_res_cache_max_age_>0)
            {
                std::string max_age = std::string("max-age=") + std::to_string(static_res_cache_max_age_);
            ...
            if(req.is_range())    //如果是范围请求，server 添加返回的数据是多少字节
            {
        ...
        void write_chunked_body(request& req) {
            const size_t len = 3 * 1024 * 1024;    //每次发送 3M 数据
        ...
        ssl_configure ssl_conf_;    //ssl配置文件
